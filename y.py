...
async def handle_gmail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    gmail_address = update.message.text

    if '@' not in gmail_address or not gmail_address.endswith('@gmail.com'):
        await update.message.reply_text("Please enter a valid Gmail address.")
        return GMAIL  # Stay in the same state if the Gmail address is invalid

    context.user_data['gmail'] = gmail_address  # Store the Gmail address
...
def main():
    token = "8040460334:AAEXO4abG0FDGohTvLq7CCWf-vY7Ff1Iglk"  # Replace with your bot token
